...
function Comment({
  ...
  const isFormVisible = formComment === id;
  ...
  return (
    <div style={{ marginLeft: `${limitedIndent}px`, wordWrap: "break-word" }}>
      <div className={cn()}>
        ...
        {commentActionState === id ? (
          <CommentAuthentication exists={exists} checkProfile={checkProfile} />
        ) : null}
        {isFormVisible && (
          <CommentForm
            title={"Новый ответ"}
            ...
            indent={indent}
            type={true}
            onChangeFormComment={onChangeFormComment}
            commentId={id}
            hand={hand2}
          />
        )}
